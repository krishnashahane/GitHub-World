...
const getDeveloper = cache(async (username: string) => {
  if (isMockMode()) {
    const mock = MOCK_DEVELOPERS.find(
      (d) => d.github_login.toLowerCase() === username.toLowerCase()
    );
    return mock ?? null;
  }
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!dev) {
    return { title: "Developer Not Found - Github World" };
  }

  const contribs = (dev.contributions_total && dev.contributions_total > 0) ? dev.contributions_total : dev.contributions;
  const title = `@${dev.github_login} - Github World | ${contribs.toLocaleString()} contributions`;
  const description = `See @${dev.github_login}'s building in Github World. ${contribs.toLocaleString()} contributions, ${dev.public_repos.toLocaleString()} repos, ${dev.total_stars.toLocaleString()} stars. Rank #${dev.rank ?? "?"} in the city.`;
...
export default async function DevPage({ params }: Props) {
  ...
  let referredDevs: { github_login: string; avatar_url: string | null }[] = [];

  if (isMockMode()) {
    // Build achievements from mock data's achievements array
    const achArr = (dev as Record<string, unknown>).achievements;
    if (Array.isArray(achArr)) {
      achievements = achArr.map((id: string) => ({
        achievement_id: id,
    ...
    achievements = (devAchievements ?? []).map((a: Record<string, unknown>) => ({
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Github World", item: baseUrl },
      {
        "@type": "ListItem",
        position: 2,
