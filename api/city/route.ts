...
import { getMockDevelopers, MOCK_CITY_STATS, isMockMode } from "@/lib/mock-data";

export async function GET(request: Request) {
  // Mock mode: return demo data without Supabase
  if (isMockMode()) {
    const devs = getMockDevelopers();
    return NextResponse.json(
      {
        developers: devs,
        stats: MOCK_CITY_STATS,
      },
      {
        headers: {
