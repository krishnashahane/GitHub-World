    ...
    const sendResults = await Promise.allSettled(
      devs.map((dev) => {
        ...
        sendNotificationAsync({
          ...
          title: `Your week in Github World: ${dev.app_streak ?? 0}-day streak, rank #${dev.rank ?? "?"}`,
          body: `Streak: ${dev.app_streak ?? 0} days. Kudos: ${weeklyKudos}. Check your weekly recap.`,
          html: `
            <p style="color: #c8e64a; font-size: 16px;">Your week in Github World</p>
            ${buildStatsTable(stats)}
            ${buildButton("Visit Github World", `${BASE_URL}/?user=${dev.github_login}`)}
          `,
