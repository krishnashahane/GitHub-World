              ...
              <button
                onClick={() => {
                  const text = raidData.success
                    ? `I just battled ${defenderLogin}'s building on Github World! ${raidData.attack_score} vs ${raidData.defense_score}`
                    : `${defenderLogin} defended my attack on Github World! ${raidData.attack_score} vs ${raidData.defense_score}`;
                  const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent("https://localhost:3001")}`;
