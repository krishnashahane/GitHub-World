...
import { isMockMode, MOCK_DEVELOPERS } from "@/lib/mock-data";

export const revalidate = 300; // ISR: regenerate every 5 min

export const metadata: Metadata = {
  title: "Leaderboard - Github World",
  description:
    "Top GitHub developers ranked by contributions, stars, repos, achievements, and referrals in Github World.",
};
...
export default async function LeaderboardPage({
  ...
  if (isMockMode()) {
    // Mock mode: sort mock developers by active tab metric
    const mockDevs = MOCK_DEVELOPERS.map((d) => ({
      github_login: d.github_login,
      name: d.name,
      avatar_url: d.avatar_url,
      contributions: d.contributions,
      contributions_total: (d as Record<string, unknown>).contributions_total as number | null,
      total_stars: d.total_stars,
      public_repos: d.public_repos,
      primary_language: d.primary_language,
      rank: d.rank,
      referral_count: ((d as Record<string, unknown>).kudos_count as number) ?? 0,
      kudos_count: ((d as Record<string, unknown>).kudos_count as number) ?? 0,
      created_at: (d as Record<string, unknown>).created_at as string | undefined,
      xp_total: (d as Record<string, unknown>).xp_total as number | undefined,
      xp_level: (d as Record<string, unknown>).xp_level as number | undefined,
    })) as Developer[];

    if (activeTab === "achievers") {
      // Use achievements array length as count
      for (const md of MOCK_DEVELOPERS) {
        const achArr = (md as Record<string, unknown>).achievements;
        achieverCounts[md.github_login] = Array.isArray(achArr) ? achArr.length : 0;
      }
      mockDevs.sort((a, b) => (achieverCounts[b.github_login] ?? 0) - (achieverCounts[a.github_login] ?? 0));
    } else if (activeTab === "contributors") {
      mockDevs.sort((a, b) => (a.rank ?? 999) - (b.rank ?? 999));
    } else if (activeTab === "stars") {
      mockDevs.sort((a, b) => b.total_stars - a.total_stars);
    } else if (activeTab === "architects") {
      mockDevs.sort((a, b) => b.public_repos - a.public_repos);
    } else if (activeTab === "recruiters") {
      mockDevs.sort((a, b) => (b.referral_count ?? 0) - (a.referral_count ?? 0));
    } else if (activeTab === "xp") {
      mockDevs.sort((a, b) => (b.xp_total ?? 0) - (a.xp_total ?? 0));
    }

    devs = mockDevs.slice(0, 50);
  } else {
    ...
    const orderColumn = activeTab === "contributors" ? "rank"
      ...
      : "contributions";
    const orderAscending = activeTab === "contributors";
      ...
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        <div className="mt-6 text-center">
          ...
          <p className="mt-3 text-xs text-muted normal-case">
            Top developers ranked in Github World
          </p>
        </div>
