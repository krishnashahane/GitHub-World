...
// track analytics events (no-op if @vercel/analytics is not installed)
...
const ERROR_MESSAGES: Record<string, { primary: (u: string) => string; secondary: string; hasRetry?: boolean; hasLink?: boolean }> = {
  ...
  "org": {
    ...
    secondary: "Github World is for individual profiles. Try searching for one of its contributors by their personal username.",
  },
  "no-activity": {
...
function HomeContent() {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("ghworld_theme");
    if (saved !== null) {
  ...
  const [, setDiscordMembers] = useState<number | null>(null);
  ...
  useEffect(() => {
    const saved = parseInt(localStorage.getItem("ghworld_rabbit_progress") ?? "0", 10) || 0;
  ...
  // Star count and community size (local demo mode)
  useEffect(() => {
    setStarCount(null);
    ...
    fetch("/api/preferences/theme")
      ...
      .then((data) => {
        if (data && typeof data.city_theme === "number" && data.city_theme >= 0 && data.city_theme <= 3) {
          ...
          localStorage.setItem("ghworld_theme", String(data.city_theme));
        }
      })
      .catch(() => { });
  }, [sessionUserId]);
  ...
  const cycleTheme = useCallback(() => {
    setThemeIndex((i) => {
      ...
      localStorage.setItem("ghworld_theme", String(next));
      if (sessionUserId) {
        fetch("/api/preferences/theme", {
          method: "PATCH",
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const localProgress = parseInt(localStorage.getItem("ghworld_rabbit_progress") ?? "0", 10) || 0;
        ...
        localStorage.setItem("ghworld_rabbit_progress", String(best));
        if (best > 0 && best < 5) {
  ...
  const onRabbitCaught = useCallback(async () => {
    ...
    if (session && claimed) {
      try {
        ...
        if (res.ok) {
          ...
          localStorage.setItem("ghworld_rabbit_progress", String(data.progress));
    ...
    localStorage.setItem("ghworld_rabbit_progress", String(newProgress));
  ...
  const reloadCity = useCallback(async (bustCache = false) => {
    ...
    try {
      const raw = localStorage.getItem("ghworld:loadout_override");
      if (raw) {
        ...
        if (Date.now() - ts < 10 * 60 * 1000) {
          ...
          localStorage.removeItem("ghworld:loadout_override");
        }
      }
    } catch { }
  ...
  const handleLoadFadeComplete = useCallback(() => {
    ...
    if (!localStorage.getItem("ghworld_intro_seen") && !hasDeepLink) {
      setIntroMode(true);
    }
  }, [searchParams]);
  ...
  // Phase 3: "Welcome to Github World"            11.0s → confetti + hold until end
  ...
  useEffect(() => {
    ...
    // Confetti shortly after "Welcome to Github World"
  ...
  const endIntro = useCallback(() => {
    ...
    setIntroPhase(-1);
    setIntroConfetti(false);
    localStorage.setItem("ghworld_intro_seen", "true");
    // Show welcome CTA for non-logged-in users who haven't seen it
    if (!session && !localStorage.getItem("ghworld_welcome_seen")) {
  ...
  useEffect(() => {
    ...
    dailyNudgeTimerRef.current = setTimeout(() => {
      try {
        const raw = localStorage.getItem("ghworld_fly_history");
  ...
  useEffect(() => {
    ...
    try {
      if (localStorage.getItem("ghworld_fly_history") || localStorage.getItem("ghworld_fly_hint_seen")) return;
    } catch { return; }
    flyHintTimerRef.current = setTimeout(() => {
      ...
      const autoDismiss = setTimeout(() => {
        ...
        try { localStorage.setItem("ghworld_fly_hint_seen", "1"); } catch { }
      }, 10000);
    ...
    <main className="relative min-h-screen overflow-hidden bg-bg font-pixel uppercase text-warm">
      ...
      <CityCanvas
        ...
        onExitFly={() => {
          ...
          try { currentPB = Math.max(currentPB, parseInt(localStorage.getItem("ghworld_fly_pb") || "0", 10) || 0); } catch { }
          ...
          if (isNewPB) {
            ...
            try { localStorage.setItem("ghworld_fly_pb", String(finalScore)); } catch { }
          }
          ...
          if (finalScore > 0) {
            try {
              ...
              localStorage.setItem("ghworld_fly_history", JSON.stringify(hist));
            } catch { }
          }
      ...
      {introMode && (
        <div className="pointer-events-none fixed inset-0 z-50">
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-center justify-center" style={{ height: "18%" }}>
            ...
            {/* Welcome to Github World (phase 3) */}
            <div
            ...
            >
              ...
              >
                Welcome to{" "}
                <span style={{ color: theme.accent }}>Github World</span>
              </p>
            </div>
          </div>
          ...
          >
            ...
            <button
              onClick={() => {
                ...
                try { localStorage.setItem("ghworld_fly_controls_seen", "1"); } catch { }
      ...
      {!flyMode && !introMode && !rabbitCinematic && (
        <div className={`pointer-events-auto fixed top-3 left-3 z-30 items-center gap-1.5 sm:gap-2 sm:left-auto sm:right-4 sm:top-4 ${exploreMode ? "hidden lg:flex" : "flex"}`}>
          ...
          >
            <span style={{ color: "#4ade80" }}>Globe</span>
          </Link>
            ...
            return (
              <div className="relative hidden sm:block">
                ...
                {codingPanelOpen && (() => {
                  ...
                  const creator = allDevs.find((d) => d.githubLogin.toLowerCase() === "krishnashahane");
                  const others = allDevs.filter((d) => d.githubLogin.toLowerCase() !== "krishnashahane");
                  const displayDevs = [
                    ...(creator ? [creator] : []),
                  ...
                  return (
                    <div className="absolute right-0 top-full mt-1 w-80 border-[3px] border-border bg-bg/95 backdrop-blur-sm">
                      ...
                      <div>
                        {displayDevs.map((dev) => {
                          const isCreator = dev.githubLogin.toLowerCase() === "krishnashahane";
                          return (
                            <button
                              key={dev.githubLogin}
                              onClick={() => {
                        ...
                        {!session ? (
                          ...
                          <div className="px-5 py-5">
                            ...
                            <div className="space-y-2.5 text-xs normal-case text-muted">
                              <p><span className="text-cream">1.</span> Install <a href="#" target="_blank" rel="noopener noreferrer" className="text-[#4ade80] hover:underline">Github World: Pulse</a> in VS Code</p>
                            ...
                            <p className="mt-1.5 text-[10px] normal-case text-muted/50">
                              Only your username and language are shared publicly. Control what&apos;s sent in VS Code Settings &gt; Github World &gt; Privacy.
                            </p>
                          </div>
                        ) : (
                          <div className="px-5 py-5">
                            ...
                            <div className="mb-4 space-y-2.5 text-xs normal-case text-muted">
                              ...
                              <p><span className="text-cream">2.</span> Install <a href="#" target="_blank" rel="noopener noreferrer" className="text-[#4ade80] hover:underline">Github World: Pulse</a> in VS Code</p>
                            ...
                            <p className="mt-3 text-[10px] normal-case text-muted/50">
                              Only your username and language are shared publicly. You can control this in VS Code Settings &gt; Github World &gt; Privacy.
                            </p>
                          </div>
                        )}
                      </div>
          ...
          {session ? (
            ...
            <div className="px-5 pt-6 pb-5 border-b border-border">
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs text-muted">GITHUB WORLD</span>
                <button
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex h-8 w-8 items-center justify-center border-2 border-border text-muted"
                >
            ...
            <div className="divide-y divide-border/40">
              ...
              >
                <span className="flex items-center gap-2 text-sm" style={{ color: "#4ade80" }}>
                  Commit Globe
                  <span className="px-1 py-px text-[8px] font-bold text-bg leading-none" style={{ backgroundColor: "#4ade80" }}>NEW</span>
                </span>
                <span className="text-xs" style={{ color: theme.accent }}>&#8594;</span>
              </Link>
              <Link
                ...
                className="flex items-center justify-between px-5 py-4 active:bg-white/5"
              >
                <span className="text-sm" style={{ color: "#a78bfa" }}>Solar System</span>
                <span className="text-xs" style={{ color: theme.accent }}>&#8594;</span>
              </Link>
              <Link
                href="/dna"
                ...
                className="flex items-center justify-between px-5 py-4 active:bg-white/5"
              >
                <span className="text-sm" style={{ color: "#f472b6" }}>Repo DNA</span>
                <span className="text-xs" style={{ color: theme.accent }}>&#8594;</span>
              </Link>
              <Link
                href="/arena"
                ...
                className="flex items-center justify-between px-5 py-4 active:bg-white/5"
              >
                <span className="text-sm" style={{ color: "#f59e0b" }}>Repo Arena</span>
                <span className="text-xs" style={{ color: theme.accent }}>&#8594;</span>
              </Link>
              <Link
                href="/archaeology"
                ...
                className="flex items-center justify-between px-5 py-4 active:bg-white/5"
              >
                <span className="text-sm" style={{ color: "#8b5cf6" }}>Code Archaeology</span>
                <span className="text-xs" style={{ color: theme.accent }}>&#8594;</span>
              </Link>
            </div>
            ...
            <div className="border-t border-border px-5 py-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {THEMES.map((t, i) => (
                  <button
                    key={t.name}
                    onClick={() => { setThemeIndex(i); try { localStorage.setItem("ghworld_theme", String(i)); } catch { } }}
              ...
              >
                ...
                <button
                  onClick={() => {
                    ...
                    localStorage.setItem("ghworld_welcome_seen", "true");
                    handleSignIn();
                  }}
                  ...
                  <div className="relative">
                    <button
                      onClick={() => {
                        ...
                        setFlyElapsedSec(0);
                        try { setFlyPersonalBest(parseInt(localStorage.getItem("ghworld_fly_pb") || "0", 10) || 0); } catch { setFlyPersonalBest(0); }
                        // Feature 3: show controls overlay on first flight
                        if (!localStorage.getItem("ghworld_fly_controls_seen")) {
                ...
                <div className="px-4 pt-3 pb-1 flex gap-2">
                  <a
                    href={`https://x.com/intent/tweet?text=${encodeURIComponent(
                      `I just compared my building with ${comparePair[1].login}'s in Github World. It wasn't even close. What's yours?`
                ...
                <div className="px-4 flex items-center gap-2 pb-1">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const a = document.createElement("a");
                      a.href = url;
                      a.download = `ghworld-${comparePair[0].login}-vs-${comparePair[1].login}.png`;
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const a = document.createElement("a");
                      a.href = url;
                      a.download = `ghworld-${comparePair[0].login}-vs-${comparePair[1].login}-stories.png`;
                      document.body.appendChild(a);
                      a.click();
                      a.remove();
                      URL.revokeObjectURL(url);
