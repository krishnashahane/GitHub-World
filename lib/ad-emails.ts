...
const FROM = "Github World <noreply@localhost:3001>";
...
export async function sendAdExpiringEmail(
  ...
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: `Your Github World ad expires in ${daysLeft} day${daysLeft === 1 ? "" : "s"}`,
    html: `
      <div style="font-family: 'Silkscreen', monospace; max-width: 480px; margin: 0 auto; padding: 24px; background: #0a0a0e; color: #f0f0f0;">
        ...
        <p>Want to keep running? <a href="https://localhost:3001/advertise" style="color: #c8e64a;">Renew your ad</a></p>
        <hr style="border-color: #1c1c20; margin: 20px 0;" />
        <p style="font-size: 12px; color: #3a3a44;">Github World - localhost:3001</p>
      </div>
    `,
  });
}
...
export async function sendAdExpiredEmail(
  ...
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: `Your Github World ad "${escapeHtml(adBrand)}" has ended`,
    html: `
      <div style="font-family: 'Silkscreen', monospace; max-width: 480px; margin: 0 auto; padding: 24px; background: #0a0a0e; color: #f0f0f0;">
